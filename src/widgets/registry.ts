...
export const WIDGETS: WidgetBlueprint[] = [
  ...
  { type: 'button', label: 'כפתור', icon: MousePointerSquareDashed, create: () => ({ type: 'button', label: 'לחצו כאן', visible: true, variant: 'filled', style: { background: '#111111', color: '#ffffff', padding: { top: 8, bottom: 8, left: 12, right: 12 } } }) },
  ...
  { type: 'banner', label: 'באנר', icon: Megaphone, create: () => ({ type: 'banner', backgroundImage: 'https://picsum.photos/1200/500', overlayColor: 'rgba(0,0,0,0.5)', heading: '<strong>מבצע!</strong>', text: 'הנחות ענק', ctaLabel: 'לפרטים', visible: true, style: { minHeight: '360px' }, contentPosition: { horizontal: 'center', vertical: 'center' }, headingStyle: { fontSize: 28, fontWeight: 600, lineHeight: 1, color: '#ffffff' }, textStyle: { fontSize: 16, lineHeight: 1, color: '#ffffff' }, buttonStyle: { background: '#2563eb', color: '#ffffff', borderRadius: 6, fontSize: 16, fontWeight: 500, padding: { top: 8, bottom: 8, left: 16, right: 16 } }, buttonWidth: 'auto', buttonAlign: 'center', buttonVariant: 'filled' } as any) },
  ...
  { type: 'productSlider', label: 'סליידר מוצרים', icon: ShoppingCart, create: () => ({ type: 'productSlider', title: 'מוצרים מומלצים', productIds: [], slidesPerView: { desktop: 4, tablet: 2, mobile: 1 }, arrows: true, dots: false, autoplay: false, imageRatio: '6/9', visible: true, style: {} } as any) },
  { type: 'newsletter', label: 'ניוזלטר', icon: Mail, create: () => ({ type: 'newsletter', visible: true, title: 'הצטרפו לניוזלטר', description: 'הטבות ומבצעים לנרשמים', placeholder: 'כתובת דוא"ל', label: 'מייל', showName: false, showPhone: false, checkboxEnabled: true, checkboxLabelHtml: 'אני מאשר/ת קבלת דיוור', buttonLabel: 'SEND', successMessage: 'תודה! נרשמת בהצלחה.', align: 'center', inputRadius: 6, buttonRadius: 6, style: { background: 'transparent', padding: { top: 24, bottom: 24 } } } as any) },
